...
description: |
  ...
  film-treatment 스킬의 규칙으로 세계관/3막 구조/인물을 설계하고,
  film-character 스킬의 지식으로 상세 캐릭터 프로파일을 생성한다.
...
`film-treatment` 스킬, `film-character` 스킬, `kling3-prompt` 스킬의 규칙을 적용하여
...
**Action**: `film-treatment` 스킬의 **로그라인 분석 규칙**에 따라 요소를 추출한다.
...
**Action**: `film-treatment` 스킬의 **세계관 설계 규칙**을 적용하여 세계관을 정의한다.
...
**Action**: `film-treatment` 스킬의 **3막 구조 규칙**을 적용하여 생성한다.
...
**Action**: `film-treatment` 스킬의 **인물 설계 규칙**을 적용하여 인물을 도출한다.
...
**Action**: `film-treatment` 스킬의 **시놉시스 규칙**을 적용하여 생성한다.
- 200-300자 범위
...
**Self-Check**: `film-treatment` 스킬의 **Quality Criteria** 기준으로 검증한다. 미충족 시 수정.
...
- 로그라인이 없으면 이 에이전트를 호출하지 않는다 (film-treatment 스킬이 사전 검증)
